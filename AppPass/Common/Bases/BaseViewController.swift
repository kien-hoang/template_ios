...
class BaseViewController: UIViewController {
    ...
    fileprivate func configButtonBack(isPresentVC: Bool) {
        ...
        if  isPresentVC {
            barButtonItem = UIBarButtonItem(image: R.image.ic_back(), style: .plain, target: self, action: #selector(BaseViewController.dismissViewController(showTabBar:)))
            navigationItem.setRightBarButton(barButtonItem, animated: true)
        } else {
            barButtonItem = UIBarButtonItem(image: R.image.ic_back(), style: .plain, target: self, action: #selector(BaseViewController.popToRootViewController(toRoot:)))
    ...
    @objc
    func popToRootViewController(toRoot: Bool = false) {
        guard let navi = self.navigationController else {
            return
        }
        hideKeyboard()
    ...
    @objc
    func dismissViewController(showTabBar: Bool = false) {
        guard let navi = self.navigationController else {
            return
        }
        hideKeyboard()
